<template>
    <div class="academicContent">
        ...
        <div class="essayBox" v-for="essay in displayEssays" :key="essay"
            ...
            <div class="essayBox-name" @click="enterEssay(essay)" style="box-shadow: none;">{{ essay.display_name }}</div>
            <div class="essayBox-author" style="box-shadow: none;">
                <span v-for="author in essay.authors" :key="author" @click="enterScholarPortal(author)">{{
                    author.display_name }},</span>
            </div>
            <div class="essayBox-abstract" style="box-shadow: none;" :title="essay.abstract">{{ essay.abstract }}</div>
            <div class="essay-indicators" style="box-shadow: none;">
                <span class="essay-indicator"><a style="color: black;">被引用数：</a>{{ essay.cited_by_count }}</span>
                <span class="essay-indicator"><a style="color: black;">发表时间：</a>{{ essay.publication_date }}</span>
                <a :href="essay.pdf_url" target="_blank"><span class="essay-indicator-op"><el-icon>
                            <Reading />
                        </el-icon>pdf预览</span></a>
        ...
        <el-dialog title="引用" v-model="citeDialogVisible" width="50%">
            <div>引用格式：</div>
            <div>
                <el-button @click="changeFormat(format)" v-for="format in ['IEEE', 'GB/T7714', 'BibText', 'Chicago']"
                    :key="format">{{ format }}</el-button>
            </div>
