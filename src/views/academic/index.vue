<template>
    <div class="academicContainer" v-loading="loadingTag" element-loading-text="拼命加载中" element-loading-background="white">
        <div v-if="!loadingTag">
            <div class="essay-name">{{ essay.title }}</div>
            <div class="essay-author">
                <span v-for="authorship in essay.authorships" :key="authorship" @click="enterScholarPortal(authorship)"
                    style="cursor: pointer;">{{ authorship.author.display_name }},</span>
            </div>
            ...
            <div class="essay-dec">
                <div>相关词条：</div>
                <div>
                    <el-tag type="info" v-for="concept in essay.concepts" :key="concept" class="concept">{{
                        concept.display_name }}</el-tag>
                    <!-- <span></span> -->
                </div>
            ...
            <div class="essay-operation">
                <el-tag class="op-share" @click="getCitation" style="cursor: pointer;"><el-icon>
                        <Link />
                    </el-icon>引用</el-tag>
                <a v-if="essay.primary_location.pdf_url != null"
                    :href="essay.primary_location ? essay.primary_location.pdf_url : null" target="_blank"><el-tag
                        class="op-read" type="success" style="cursor: pointer;"><el-icon>
                            <Reading />
                        </el-icon>pdf预览</el-tag></a>
                <el-tag v-if="essay.primary_location.pdf_url != null" class="op-share" type="success"
                    @click="downloadPDF(essay.primary_location ? essay.primary_location.pdf_url : null)"
                    style="cursor: pointer;"><el-icon>
                        <Download />
                    </el-icon>pdf下载</el-tag>
                <a :href="essay.primary_location ? essay.primary_location.landing_page_url : null" target="_blank"><el-tag
                        class="op-read" type="danger" style="cursor: pointer;"><el-icon>
                            <View />
                        </el-icon>阅读</el-tag></a>
                <el-tag class="op-collection" @click="essayCollection" type="warning" style="cursor: pointer;"><el-icon>
                        <Star />
                    </el-icon>收藏</el-tag>
            </div>
    ...
    <el-dialog title="引用" v-model="citeDialogVisible" width="50%">
        <div>引用格式：</div>
        <div>
            <el-button @click="changeFormat(format_)" v-for="format_ in ['IEEE', 'GB/T7714', 'BibText', 'Chicago']"
    ...
    <!-- 收藏对话框 -->
    <el-dialog title="选择收录文献的收藏夹" v-model="collectionDialogVisible" width="30%" :destroy-on-close="true"
        :show-close="false">
        <el-radio-group v-model="collectionId" @change="collectionChange">
            <el-radio v-for="collection in collections" :key="collection" :label="collection.id">{{ collection.name
            }}</el-radio>
        </el-radio-group>
        <div class="dialog-footer">
    ...
    <el-dialog title="新标签名称" v-model="addCollectionVisible" width="30%">
